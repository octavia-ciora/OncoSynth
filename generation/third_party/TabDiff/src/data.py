...
class dequantizer:
    ...
    def transform(self, X):
        ...
        if self.dequant_dist == "uniform":
            X[:, self.int_col_idx_wrt_num] = (
                X_int + np.random.uniform(size=X_int.shape) * self.int_dequant_factor
            )
        elif self.dequant_dist == "beta":
            X[:, self.int_col_idx_wrt_num] = (
                X_int
                + np.random.beta(
                    self.int_dequant_factor, self.int_dequant_factor, size=X_int.shape
                )
                - 0.5
            )
...
def transform_dataset(
    ...
    if (
        X_num is not None
        and int_col_idx_wrt_num
        and transformations.dequant_dist is not None
    ):
    ...
    if dataset.X_cat is None:
        ...
        if not has_cat:
            ...
            for (
                ...
                X_cat.keys()
            ):  # a patch to make sure that the empty array is transformed into int dtype
