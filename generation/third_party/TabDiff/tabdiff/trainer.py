...
class Trainer:
    ...
    def run_loop(self):
        ...
        for epoch in range(start_epoch, self.steps):
            ...
            if len(self.dataset.categories) > 0:  # categorical data is not empty
                ...
                ):  # non-learnable cat schedule
                    cat_noise_dict = {
                        "cat_noise/k": self.diffusion.cat_schedule.k().item()
                    }
                else:
                    cat_noise_dict = {
            ...
            update_ema(
                self.ema_num_schedule.parameters(),
                self.diffusion.num_schedule.parameters(),
                rate=self.ema_decay,
            )
            update_ema(
                self.ema_cat_schedule.parameters(),
                self.diffusion.cat_schedule.parameters(),
                rate=self.ema_decay,
            )

            # Save ckpt base on the best training loss
            if total_loss < best_loss and self.curr_epoch > self.save_best_after:
                best_loss = total_loss
                to_remove = glob.glob(
                    os.path.join(self.model_save_path, f"best_model_*")
                )
                if to_remove:
            ...
            if (
                ...
                and self.curr_epoch > self.save_best_after
            ):
                best_ema_loss = ema_total_loss
                to_remove = glob.glob(
                    os.path.join(self.model_save_path, f"best_ema_model_*")
                )
                if to_remove:
                    os.remove(to_remove[0])
                state_dicts = {
            ...
            if (epoch + 1) % self.check_val_every == 0:
                ...
                print_with_bar(
                    f"Routine Generation Evaluation every {self.check_val_every}, currently at epoch #{epoch+1}, wiht total_loss={total_loss}."
                )
        ...
        end_time = time.time()
        print_with_bar(
            f"Ending Trainnig Loop, totoal training time = {end_time - start_time}"
        )
    ...
    def evaluate_generation(
        ...
        num_samples = (
            ...
            else self.metrics.real_data_size
        )  # By default, num_samples_to_generate is not specified. In this case, we generate the same number of samples as the real dataset. This approach is consistently used across all experiments in the paper.
    ...
    def sample_synthetic(self, num_samples, keep_nan_samples=True, ema=False):
        ...
        end_time = time.time()
        print_with_bar(
            f"Ending Sampling, totoal sampling time = {end_time - start_time}"
        )
    ...
    def test_impute(
        ...
        # Use dataset info directly so impute doesn't depend on TabMetrics/CSV files.
        info = getattr(self.dataset, "info", None)
        if info is None:
            raise ValueError("Dataset info not available; cannot run imputation.")
        ...
        with torch.no_grad():
            ...
            for trial in range(trail_start, trail_start + trial_size):
                ...
                (
                    os.makedirs(imputed_sample_save_dir)
                    if not os.path.exists(imputed_sample_save_dir)
                    else None
                )
                print(
