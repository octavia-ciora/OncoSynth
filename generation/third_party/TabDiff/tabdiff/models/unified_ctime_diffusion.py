...
class UnifiedCtimeDiffusion(torch.nn.Module):
    ...
    def _subs_parameterization(self, unormalized_prob, xt):
        ...
        unormalized_prob[
            :, range(unormalized_prob.shape[1]), self.mask_index
        ] += self.neg_infinity
    ...
    def edm_update(
        ...
        x_cat_hat, _ = (
            self.q_xt(x_cat_cur, move_chance) if has_cat else (x_cat_cur, x_cat_cur)
        )

        # Get predictions
        x_cat_hat_oh = (
            self.to_one_hot(x_cat_hat).to(x_num_hat.dtype) if has_cat else x_cat_hat
        )
        ...
        if self.sampler_params["second_order_correction"]:
            if i > 0:
                x_cat_hat_oh = (
                    self.to_one_hot(x_cat_hat).to(x_num_next.dtype)
                    if has_cat
                    else x_cat_hat
                )
                denoised, raw_logits = self._denoise_fn(
