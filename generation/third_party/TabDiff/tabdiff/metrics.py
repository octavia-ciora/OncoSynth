...
class TabMetrics(object):
    def __init__(
        self, real_data_path, test_data_path, val_data_path, info, device, metric_list
    ) -> None:
    ...
    def evaluate_mle(self, syn_data):
        ...
        if task_type == "regression":
            ...
            (
                ...
                best_acc_scores,
                best_avg_scores,
            ) = evaluator(train, test, info, val=val)
    ...
    def evaluate_dcr(self, syn_data):
        ...
        cat_complete_data_np = np.concatenate(
            [cat_real_data_np, cat_test_data_np], axis=0
        )
        ...
        dcrs_test = []
        batch_size = (
            10000 // cat_real_data_oh.shape[1]
        )  # This esitmation should make sure that dcr_real and dcr_test can be fit into 10GB GPU memory
    ...
    def plot_density(self, syn_data):
        ...
        if y_only:
            ...
            syn_data_cp = self.complete_y_only_data(
                syn_data_cp, real_data, target_col_name
            )
