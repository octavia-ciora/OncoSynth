...
class TabDiffDataset(Dataset):
    def __init__(
        ...
        X_num, X_cat, categories, d_numerical, num_inverse, int_inverse, cat_inverse = (
            preprocess(
                data_dir,
                y_only,
                dequant_dist,
...
def preprocess(
    ...
    if cat_encoding is None:
        ...
        if inverse:
            num_inverse = (
                dataset.num_transform.inverse_transform
                if dataset.num_transform is not None
                else lambda x: x
            )
            int_inverse = (
                dataset.int_transform.inverse_transform
                if dataset.int_transform is not None
                else lambda x: x
            )
            cat_inverse = (
                dataset.cat_transform.inverse_transform
